...
- **CSS/JSのキャッシュ対策**: 各ページの`css/style.css` / `js/main.js`の読み込みには`?v=4`というバージョン番号を付けています。ファイル名が変わらないと訪問者のブラウザに古いキャッシュが残り続け、「新しいHTML＋古いCSS/JS」の組み合わせで表示が崩れることがあります（実際に、フッターのSNSアイコン追加時にこれが原因でアイコンが巨大な黒い図形として表示される不具合がありました）。`css/style.css`または`js/main.js`の中身を変更したときは、4ページすべての番号を1つ上げてください（`?v=3`, `?v=4`...）。番号を上げ忘れても実害はありませんが、訪問者が変更に気付くまで時間がかかることがあります。
- **Xタイムライン埋め込み（links.html）**: `.x-timeline`内の`<a class="twitter-timeline">`が公式の埋め込みタイムラインです。ユーザー名を変えたい場合は`href`とリンクテキストの`hotChicken_sp`部分を書き換えてください。X社の外部スクリプト（`platform.twitter.com/widgets.js`）を読み込む唯一の外部連携で、下記セキュリティ注意点も参照してください。
  - モバイルの一部ブラウザ（アプリ内ブラウザや広告/トラッキングブロック機能付きブラウザなど）ではこの外部スクリプトがブロックされ、タイムラインが表示されないことがあります。その場合`<a class="twitter-timeline">`のリンク文字列がそのままボタン風に表示される作り（`.x-timeline a.twitter-timeline`のCSS）になっているので、壊れて見えることはありません。
...
- **外部スクリプト・埋め込みを追加する際は信頼できるものだけを使う**: 現状読み込んでいる外部リソースはGoogle Fonts（`fonts.googleapis.com` / `fonts.gstatic.com`）と、links.htmlのXタイムライン埋め込み用スクリプト（`platform.twitter.com`）です。いずれもHTTPSで取得していますが、Xのウィジェットは提供元（X社）にアクセス情報が渡る外部連携なので、その点を理解した上で使用してください。今後アクセス解析タグや埋め込みウィジェットなどを追加する場合も、信頼できる提供元かつHTTPS配信のものに限定してください。
